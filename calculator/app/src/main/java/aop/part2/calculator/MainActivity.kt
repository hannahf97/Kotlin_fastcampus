...
class MainActivity : AppCompatActivity() {
    ...
    fun buttonClicked(v: View) {
        ...
        when(v.id) {
            ...
            R.id.modulorButton ->  operatorButtonClicked("%")
    ...
    private fun numberButtonClicked(number: String){
        ...
        if(expressionText.isNotEmpty() && expressionText.last().length >= 15){
            ...
            Toast.makeText(this, "0은 젤 앞에 올수 없습니다",Toast.LENGTH_SHORT).show()
            return
        }
        expressionTextView.append(number)
        resultTextView.text = calculateExpression()
        //realtextview에 실시간으로 계산결과 넣어야햄
    ...
    fun resultButtonClicked(v: View) {
        ...
        if (expressionTexts[0].isNumber().not() || expressionTexts[2].isNumber().not()){
            Toast.makeText(this, "오루가 발생했습니다. ",Toast.LENGTH_SHORT).show()
            return
        }
    ...
    fun historyClosedButtonClicked(v:View){
